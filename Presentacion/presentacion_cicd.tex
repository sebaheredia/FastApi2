...
git commit --amend   # modificar el ultimo
    ...
    \column{0.5\textwidth}
    \begin{block}{El flujo basico}
      \begin{center}
        \begin{tikzpicture}[node distance=0.5cm]
          \node[fill=gris!30, rounded corners,
                minimum width=3cm, minimum height=0.7cm] (wd)
                {Working Directory};
          ...
          \draw[->, thick] (wd) -- node[right]{\tiny git add} (sa);
          \draw[->, thick] (sa) -- node[right]{\tiny git commit} (repo);
        \end{tikzpicture}
      \end{center}
    \end{block}
  \end{columns}
...
\begin{frame}{GitHub vs Git}
  ...
  \begin{alertblock}{Analogia}
    Git es el motor del auto. GitHub es la autopista donde circula.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Issues --- Gestion de tareas}
  Los \textbf{Issues} son el sistema de seguimiento de tareas de GitHub.
...
\begin{frame}{Que son los Workflows?}
  ...
  \begin{block}{Caracteristicas}
    \begin{itemize}
      ...
      \item Se activan por eventos: push, PR, horario, manual
...
\begin{frame}[fragile]{Pipeline completo: test -> build -> deploy}
  \begin{lstlisting}[style=yaml]
jobs:
